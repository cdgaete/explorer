  ...
  return (
    <PanelResizeHandle
      className={cn(
        ...
        direction === 'horizontal' ? "w-0.5 hover:bg-primary/50" : "h-0.5 hover:bg-primary/50",
...
function SidebarResizeHandle({ onToggle, isCollapsed }: { onToggle: () => void; isCollapsed: boolean }) {
  return (
    <PanelResizeHandle
      className="relative w-0.5 transition-colors hover:bg-primary/50 before:absolute before:inset-0 before:-left-1 before:-right-1"
    >
      <button
        onClick={(e) => {
          e.stopPropagation()
